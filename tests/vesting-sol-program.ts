...
describe("vesting-sol-program", async () => {
  ...
  it("Create Vesting Record for Alice (10 periods)", async () => {
    ...
    const startTime = new anchor.BN(currentTime + 60); // Starts in 60 seconds
    const endTime = new anchor.BN(currentTime + 10060); // Ends in ~2.8 hours
    ...
    // Verify treasury received creation fee
  ...
  it("Different users can create vesting records with same vesting account parameters", async () => {
    ...
    // Dave creates vesting record using Alice's vesting account (same parameters as Alice)
    await program.methods
      .createVesting(
      ...
      .accounts({
        ...
        vesting: vestingAccountAlice.publicKey, // Using Alice's vesting account
        treasury: treasury.publicKey,
      })
  ...
  it("Can create multiple vesting records with different configurations", async () => {
    ...
    // First vesting: 100 periods, small amounts
    await program.methods
      .createVesting(
    ...
    // Second vesting: 1 period, large amount
    await program.methods
      .createVesting(
